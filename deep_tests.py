...
class AccountsDeepTest(TestCase):
    ...
    def test_db_uuid_is_unique_per_user(self):
        u1 = User.objects.create_user(email="u1@x.com", password="Pass@1234")
        u2 = User.objects.create_user(email="u2@x.com", password="Pass@1234")
    ...
    def test_db_role_choices_enforced_at_db(self):
        # Django doesn't enforce choices at DB level, but serializer should
        u = User.objects.create_user(email="role@x.com", password="Pass@1234", role="INVALID")
        # This will save — choices only enforced at form/serializer level
        # So confirm the value is stored (this is expected Django behavior)
        self.assertEqual(u.role, "INVALID")
    ...
    def test_api_created_user_retrievable_from_db(self):
        ...
        self.assertEqual(u.first_name, "JOHN")  # uppercased by serializer
    ...
    def test_api_phone_numeric_only_enforced(self):
        for phone in ["abc123", "98+76543", "98 765 43"]:
            ...
            if res.status_code != 400:
                print(f"\n[WARN] Phone '{phone}' was accepted but should be rejected")
                print(f"  Response: {res.data}")
            self.assertEqual(res.status_code, 400, f"Phone '{phone}' should be rejected")
    ...
    def test_api_all_roles_accepted(self):
        for role in ["ORGANIZER", "TEAMHEAD", "UMPIRE", "VIEWER"]:
            payload = {
                ...
                "first_name": "A", "last_name": "B", "phone": "1234567890", "role": role
            }
            res = self.client.post("/api/accounts/users/", payload, format="json")
            if res.status_code != 201:
                print(f"\n[FAIL] Role '{role}' rejected: {res.data}")
    ...
    def test_api_jwt_token_contains_user_id(self):
        ...
        print(f"\n  JWT payload keys: {list(decoded.keys())}")
...
class VenuesDeepTest(TestCase):
    ...
    def test_db_venue_count_after_create(self):
        ...
        Venue.objects.create(**self.payload)
    ...
    def test_db_venue_update_persists(self):
        v = Venue.objects.create(**self.payload)
    ...
    def test_api_multiple_venues_list_count(self):
        Venue.objects.create(**{**self.payload, "name": "Ground A"})
        Venue.objects.create(**{**self.payload, "name": "Ground B"})
    ...
    def test_api_invalid_gps_rejected(self):
        ...
        if res.status_code != 400:
            print(f"\n[WARN] Invalid GPS accepted: {res.data}")
...
class PlayersDeepTest(TestCase):
    ...
    def test_api_filter_active_players_only(self):
        make_player(name="Active One")
        p2 = make_player(name="Inactive One")
        p2.is_active = False
        p2.save()
        res = self.client.get("/api/players/?is_active=True")
        assertOK(self, res, 200)
        print(f"\n  Active players returned: {len(res.data)}")
...
class TeamsDeepTest(TestCase):
    ...
    def test_api_squad_response_has_correct_fields(self):
        ...
        assertOK(self, res, 201)
        print(f"\n  Squad response fields: {list(res.data.keys())}")
    ...
    def test_api_squad_role_choices(self):
        ...
        for i, role in enumerate(["PLAYER", "CAPTAIN", "VICE_CAPTAIN"]):
            ...
            if res.status_code != 201:
                print(f"\n[FAIL] Squad role '{role}' rejected: {res.data}")
            self.assertEqual(res.status_code, 201)
