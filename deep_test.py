...
class DeepSystemArchitectureTest(TestCase):
    ...
    def setUp(self):
        ...
        Application.objects.create(team=self.teamA, tournament=self.tournament, status="ACCEPTED")
        Application.objects.create(team=self.teamB, tournament=self.tournament, status="ACCEPTED")
    ...
    def test_automation_e2e_scoring_and_standings_sync(self):
        ...
        # Instantiate standing records row rows
        TournamentStanding.objects.create(tournament=self.tournament, team=self.teamA)
        TournamentStanding.objects.create(tournament=self.tournament, team=self.teamB)
